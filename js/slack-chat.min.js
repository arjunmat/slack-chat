...
type:"POST",dataType:"json",data:{token:options.apiToken,channel:options.channelId,oldest:options.latest,count:options.messageFetchCount},success:function(a){options.debug&&a.messages.length&&console.log(a.messages);if(a.ok&&a.messages.length){var b="";options.latest=a.messages[0].ts;a.messages=a.messages.reverse();for(var f=0;f<a.messages.length;f++)if("bot_message"==a.messages[f].subtype&&""!==a.messages[f].text){message=a.messages[f];var e="",g="",h="";message.attachments&&(e=message.attachments[0].author_name,
g=message.attachments[0].author_icon);message.fields&&(h=message.fields[0].value);var k=message.text.trim(),b=b+"<div class='message-item'>";""!==g&&(b+="<div class='userImg'><img src='"+g+"' /></div>");b+="<div class='msgBox'>";b=""!==h?b+("<div class='username'>"+(h==options.userId?"You":e)+"</div>"):b+("<div class='username'>"+e+"</div>");b+="<div class='message'>"+k+"</div>";"undefined"!==typeof moment&&(b+="<div class='timestamp'>"+moment.unix(a.messages[f].ts).fromNow()+"</div>");b+="</div>";
b+="</div>"}else"undefined"==typeof a.messages[f].subtype&&(message=a.messages[f].text,e=options.sysUser,k=message,b+="<div class='message-item'>",""!==options.sysImg&&(b+="<div class='userImg'><img src='"+options.sysImg+"' /></div>"),b+="<div class='msgBox'>",b+="<div class='username main'>"+e+"</div>",b+="<div class='message'>"+k+"</div>","undefined"!==typeof moment&&(b+="<div class='timestamp'>"+moment.unix(a.messages[f].ts).fromNow()+"</div>"),b+="</div>",b+="</div>");c(".slack-message-box").append(b);
c(".slack-message-box").stop().animate({scrollTop:c(".slack-message-box")[0].scrollHeight},800)}else a.ok||(console.log("[SlackChat] Query failed with errors: "),console.log(a))}})},postMessage:function(a){a=a._options;var d={};d.fallback="View "+a.user+"'s profile";d.color=a.slackColor;d.author_name=a.user;""!==a.userLink&&(d.author_link=a.userLink);""!==a.userImg&&(d.author_icon=a.userImg);""!==a.userId&&(d.fields=[{title:"ID",value:a.userId,"short":!1}]);message=c(".slack-new-message").val();c(".slack-new-message").val("");
a.debug&&(console.log("Posting Message:"),console.log({message:message,attachment:d,options:a}));c.ajax({url:"https://slack.com/api/chat.postMessage",type:"POST",dataType:"json",data:{token:a.apiToken,channel:a.channelId,text:message,username:a.botUser,attachments:JSON.stringify([d])},success:function(a){a.ok||(c(".slack-new-message").val(message),console.log("[SlackChat] Post Message failed with errors: "),console.log(a))}})},validationError:function(a){c.error("[SlackChat Error] "+a);return!1},
getUserPresence:function(a){var d=a._options,b=[];c.ajax({url:"https://slack.com/api/users.list",type:"POST",dataType:"json",data:{token:d.apiToken},success:function(a){if(a.ok&&(b=a.members,b.length))for(a=0;a<b.length;a++)b[a].is_bot||c.ajax({url:"https://slack.com/api/users.getPresence",dataType:"json",type:"POST",data:{token:d.apiToken,user:b[a].id},success:function(a){if(a.ok){if("active"===a.presence)return c(".slackchat .presence").addClass("active"),c(".slackchat .presence .presence-text").text("Available"),
d.disableIfAway&&null!==d.elementToDisable&&d.elementToDisable.show(),!0;c(".slackchat .presence").removeClass("active");c(".slackchat .presence .presence-text").text("Away")}}})}})},destroy:function(a){c(a).unbind("click");c(".slackchat").remove()}};c.fn.slackChat=function(a){if(e[a])return e[a].apply(this,Array.prototype.slice.call(arguments,1));"object"!==typeof a&&a?c.error("Method "+a+" does not exist on jQuery.slackChat"):e.init.apply(this,arguments)}})(jQuery);